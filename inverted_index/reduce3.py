...
def reduce_one_group(key, group):
    ...
    for term in seen_terms:
        norm = norm + ((seen_terms[term] * float(term_idfk[term])) ** 2)
    
    # handle output
    for word in seen_terms:
        print(f'{word}\t{term_idfk[word]} {key} {seen_terms[word]} {norm}')
